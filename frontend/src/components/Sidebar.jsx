    ...
    <aside className="w-60 h-full bg-ink-950 border-r border-ink-800/60 flex flex-col">
      <div className="px-5 py-5 border-b border-ink-800/40">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-xl bg-ink-800 border border-ink-700/50 flex items-center justify-center text-lg">📚</div>
          <div>
            <h1 className="font-display text-base font-bold text-paper-100">AirNotes</h1>
            <div className="flex items-center gap-1 mt-0.5">
              {state.demoMode
                ? <><WifiOff size={9} className="text-amber-500" /><span className="text-amber-500 text-[10px]">Demo Mode</span></>
                : <><Wifi size={9} className="text-emerald-500" /><span className="text-emerald-500 text-[10px]">Connected</span></>}
            </div>
          </div>
        </div>
      </div>
      <div className="px-3 py-3">
        <button onClick={onSearch} className="w-full flex items-center gap-2 px-3 py-2 rounded-lg bg-ink-800/50 border border-ink-700/30 text-ink-400 text-sm hover:bg-ink-800 hover:text-ink-200 transition-all">
          <Search size={14} />
          <span className="flex-1 text-left text-xs">Search PDFs…</span>
          <kbd className="text-[10px] bg-ink-700/50 px-1.5 py-0.5 rounded text-ink-500">⌘K</kbd>
        </button>
      </div>
      <nav className="px-3 space-y-0.5">
        {navItems.map(item => (
          <button key={item.key} onClick={() => actions.setActiveSection(item.key)}
            ...
            {item.count > 0 && <span className="text-[10px] bg-ink-700/60 text-ink-400 px-1.5 py-0.5 rounded-full">{item.count}</span>}
          </button>
        ))}
      </nav>
      <div className="px-3 mt-4 flex-1 overflow-y-auto">
        <div className="flex items-center justify-between mb-2 px-1">
          ...
          <button onClick={() => setCreatingFolder(true)} className="text-ink-600 hover:text-ink-300"><Plus size={13} /></button>
        </div>
        {creatingFolder && (
          <div className="mb-2 flex gap-1">
            ...
            <button onClick={createFolder} className="text-ink-300 px-1">✓</button>
          </div>
        )}
        {rootFolders.length === 0 && !creatingFolder && <p className="text-ink-700 text-xs px-1 py-2">No folders yet</p>}
...
function FolderItem({ folder, allFolders, isActive, expanded, renamingId, renameValue, onRenameChange, onToggle, onSelect, onStartRename, onRename, onDelete, fileAssignments }) {
  ...
  return (
    <div>
      <div onClick={onSelect} className={`relative group flex items-center gap-1.5 px-2 py-1.5 rounded-lg cursor-pointer text-sm transition-all ${isActive ? 'bg-ink-800 text-ink-100' : 'text-ink-400 hover:text-ink-200 hover:bg-ink-800/50'}`}>
        {children.length > 0 && <button onClick={e => { e.stopPropagation(); onToggle(); }} className="text-ink-600">{expanded ? <ChevronDown size={11} /> : <ChevronRight size={11} />}</button>}
        ...
        <div className="relative opacity-0 group-hover:opacity-100">
          ...
          {showMenu && (
            <div className="absolute right-0 top-full mt-1 z-50 glass rounded-lg shadow-xl min-w-[120px] py-1" onMouseLeave={() => setShowMenu(false)}>
              <button onClick={e => { e.stopPropagation(); onStartRename(folder.id, folder.name); setShowMenu(false); }} className="w-full text-left px-3 py-1.5 text-xs text-ink-300 hover:bg-ink-700/50">Rename</button>
              <button onClick={e => { e.stopPropagation(); onDelete(folder.id); setShowMenu(false); }} className="w-full text-left px-3 py-1.5 text-xs text-red-400 hover:bg-red-500/10">Delete</button>
            </div>
          )}
        </div>
      </div>
      {expanded && children.map(child => (
        <div key={child.id} className="ml-4 border-l border-ink-800/60 pl-2">
